...
interface OrderData {
  ...
  timeSensitiveDeadline?: Date | string | null;
  status?: 'pending' | 'scheduled' | 'completed' | 'cancelled';
}

class Order {
  static async getAll(filters: OrderFilters = {}): Promise<any[]> {
    let query = `
  ...
  static async getById(id: number): Promise<OrderRow | undefined> {
    const result = await pool.query('SELECT * FROM orders WHERE id = $1', [id]);
    return result.rows[0] as OrderRow | undefined;
  }

  static async create(orderData: OrderData): Promise<OrderRow> {
    const {
    ...
    const calculatedScore = priorityScore || this.calculatePriorityScore(priority || 'routine', isTimeSensitive || false);
  ...
  static calculatePriorityScore(priority: string, isTimeSensitive: boolean): number {
  ...
  static async updateAssignedSite(orderId: number, siteId: number, routingReason: string): Promise<OrderRow> {
    const result = await pool.query(
      `UPDATE orders 
  ...
  static async getTimeSensitiveOrders(deadlineThreshold: Date | string): Promise<OrderRow[]> {
    const result = await pool.query(
      `SELECT * FROM orders 
  ...
  static async getCombinableOrders(patientId: string, siteId: number): Promise<OrderRow[]> {
    const result = await pool.query(
      `SELECT * FROM orders 
